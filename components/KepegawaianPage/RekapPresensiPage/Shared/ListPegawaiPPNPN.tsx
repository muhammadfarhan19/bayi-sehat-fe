...
import { type GetPegawaiListData, type GetPegawaiListReq } from '../../../../types/api/KepegawaianAPI';
...
import { type ListPegawaiProps } from './types/_sharedType';

function ListPegawaiPPNPN(props: ListPegawaiProps) {
  const { unit_kerja_id, pageTitle = '', onSelectEachPegawai, CTATitle = '' } = props;
  ...
  return (
    <>
      <div className="mb-5 flex flex-row items-center px-4 pt-5">
        <h3 className="text-xl font-medium leading-6 text-gray-900">{pageTitle}</h3>
                    ...
                    {(ppnpnList?.list || []).map((data, dataIdx) => (
                      ...
                      >
                        ...
                        <td className="text-\xs px-6 py-4 font-medium">
                          <button
                            onClick={() => onSelectEachPegawai(data)}
                            type="button"
