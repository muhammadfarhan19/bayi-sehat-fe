      ...
      <label htmlFor="nama" className={`block text-sm font-medium text-gray-700 ${props.additionalLabelStyle}`}>
        ...
        {clipBoard ? (
          <div className="flex flex-row items-center space-x-1">
            ...
            <button disabled={props.disableClipboard} type="button" onClick={props.onCopyToClipboard}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke={props.disableClipboard ? 'currentColor' : 'rgb(22 163 74)'}
                className={classNames(animateClipboard ? 'animate-bounce' : '', 'h-5 w-5')}
              >
                <path
                  strokeLinecap="round"
