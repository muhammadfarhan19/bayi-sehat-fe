...
function DetailDinasPegawaiData() {
  const { status, dinas_id } = getQueryString<{ status: string; dinas_id: string }>();
  ...
  const handleAjukanPembayaran = () => {
    updateStatus(statusPembayaranId === 0 ? +2 : +1);
  };
  ...
  return (
    <>
      <div className="flex flex-col gap-y-[24px]">
        ...
        {statusPembayaranId > 1 && <SubmissionHistory status={statusPembayaranId} dinas_id={dinas_id} />}
        ...
        {[0, 1].includes(statusPembayaranId) && (
          <div className="flex flex-col items-end gap-y-2 rounded-lg bg-white py-4 shadow">
            <div className="flex items-center gap-x-2 px-6">
              <Link href="/keuangan/daftar-dinas">
                <button className="rounded border border-transparent bg-red-600 p-2 text-sm font-medium text-white shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:bg-gray-500 disabled:text-gray-200">
                  Kembali
                </button>
              </Link>
              <button
