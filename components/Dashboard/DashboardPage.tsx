...
const labelsGolongan = ['II/A', 'II/B', 'II/C', 'II/D', 'III/A', 'III/B', 'III/C', 'III/D', 'IV/A', 'IV/B', 'IV/C', 'IV/D'];
                      ...
                      <tbody className="divide-y divide-gray-200 bg-white">
                        {dummyPegawai.map((person, index) => (
                          <tr key={index}>
                            ...
                            <td className="whitespace-nowrap px-6 py-4 text-xs text-center text-gray-500">{person.totalPns}</td>
                            <td className="whitespace-nowrap px-6 py-4 text-xs text-center text-gray-500">{person.totalPpnpn}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      <section aria-labelledby="section-1-title">
        <div className="overflow-hidden rounded-lg bg-white shadow">
          <div className="p-6">
            ...
            <p className="text-sm leading-8 font-light text-gray-500">Grafik sebaran pegawai di unit kerja sekretariat direktorat jenderal pendidikan tinggi</p>
              ...
              <Listbox value={selectedPendidikan} onChange={setSelectedPendidikan}>
                {({ open }) => (
                  <>
                    <div className="mt-1 relative">
                      <Listbox.Button className="bg-white relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        <span className="block truncate text-sm leading-5 text-gray-700">{selectedPendidikan.name}</span>
                        <span className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
                          <SelectorIcon className="h-5 w-5 text-gray-400" aria-hidden="true" />
                        </span>
                      </Listbox.Button>
